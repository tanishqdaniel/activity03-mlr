...
```{r data-info}
about_page_url <- "https://www.openintro.org/data/index.php?data=hfi"
browseURL(about_page_url)

summary(hfi_2016$pf_score)
summary(hfi_2016$pf_expression_control)
summary(hfi_2016$ef_score)
```
...
hfi_2016 %>% 
  select(pf_score, pf_expression_control, ef_score) %>% 
  ggpairs()
```
...
mlr_mod <- lm_spec %>% 
fit(pf_score ~ pf_expression_control + ef_score, data = hfi_2016)
...
$$
$$
\hat{y} = 1.6987659 + ( 0.4552686 \times x_1 ) + ( 0.4439395 \times x_2 )
$$
